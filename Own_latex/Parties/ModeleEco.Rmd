...
X=select(biens_stat_des,-c(prix,maison))
```
...
Nous remarquons aussi que les variables ```arrondissement``` (qui décrit la zone) et ```nb_photo``` ne sont pas significatives (au seuil de 5%).
...
Moyenne_ORSAY_CHAPELLE_var=rowMeans(biens_stat_des[,c(15,17)])
...
```{r}
X=select(biens_stat_des_Moyenne,-c(prix,maison,arrondissement,nb_photo,distance_université_plus_près,distance_station_0,`Basilique du Sacré-Cœur de Montmartre`))
lm_2_1=lm(Y~.,data=X)  
summary(lm_2_1)
```
### Fit Sous-modèle 2 : ```Musée d'Orsay```
```{r}
X=select(biens_stat_des_ORSAY,-c(prix,maison,arrondissement,nb_photo,distance_université_plus_près,distance_station_0,`Basilique du Sacré-Cœur de Montmartre`))
lm_2_2=lm(Y~.,data=X)  
summary(lm_2_2)
```
### Fit Sous-modèle 3 : ```Chapelle Notre-Dame de la Médaille miraculeuse```
```{r}
X=select(biens_stat_des_Chapelle,-c(prix,maison,arrondissement,nb_photo,distance_université_plus_près,distance_station_0,`Basilique du Sacré-Cœur de Montmartre`))
lm_2_3=lm(Y~.,data=X)  
summary(lm_2_3)
```
...
Nous allons supprimer les variables qui ont une p-value très élevée. C'est à dire : ```Cathédrale Notre-Dame de Paris```,```Centre Pompidou```,```distance_station_1```

```{r}
X=select(X,-c(`Centre Pompidou`,`Cathédrale Notre-Dame de Paris`))
...
##Modèle avec  Rapport
#resettest(lm3,power = 2:3,type = 'regressor',data=biens_2)
```
